...
画面のスクリーンショット](images/getting_started/rails_welcome.png)
...
app/views/comments/ | コメントのビューはここに置かれる
test/controllers/comments_controller_test.rb | コントローラのテスト用ファイル
app/helpers/comments_helper.rb | ビューヘルパー
app/assets/stylesheets/comment.scss | コメント用のCSS（カスケーディングスタイルシート）ファイル
